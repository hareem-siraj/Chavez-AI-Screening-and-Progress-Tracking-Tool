...
# # Manually set the column order (temporary fix)
# feature_columns = [
#     'response_latency', 'speech_confidence', 'speech_onset_delay', 'echolalia_score',
...
#     return feature_values, mfcc_mean.tolist(), response_latency, speech_confidence, speech_onset_delay, echolalia_score
...
#     # Extract speech features
#     feature_values, mfcc_mean, response_latency, speech_confidence, speech_onset_delay, echolalia_score = extract_features(combined_audio_path)
...
#         "speech_onset_delay": speech_onset_delay,
#         "echolalia_score": echolalia_score
#     }

#     for i in range(13):
#         feature_dict[f"mfcc_{i}"] = mfcc_mean[i]

#     # The model expects only features (excluding session_id and label)
#     input_features_df = pd.DataFrame([feature_dict])[feature_columns]

#     # Predict ASD or Neurotypical
#     prediction = rf_model.predict(input_features_df)[0]
#     label = "Autistic" if prediction == 1 else "Neurotypical"
