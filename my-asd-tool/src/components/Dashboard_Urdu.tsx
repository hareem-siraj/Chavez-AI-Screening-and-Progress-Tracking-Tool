         ...
         <Grid item xs={12} md={6}>
          ...
          <Grid item xs={12} sx={{ position: 'relative', '&::after': { display: 'none' } }}>
            {!sessionStarted ? (
              ...
              >
                {createActionButton("ASD سوالنامہ", "/questionnaire-urdu", isQuestionnaireAvailable, isQuestionnaireCompleted, true)}
                {createActionButton("گیمیفائیڈ اسیسمنٹس", "/game-selection-urdu", isGameSelectionAvailable, isGameSelectionCompleted, true)}
                {createActionButton("Speech Analysis", "/audio-analysis", isSpeechAnalysisAvailable, isSpeechAnalysisCompleted, true)}
                {createActionButton("رپورٹس", "/reports", isReportsAvailable, false, true)}
              </Box>
            )}
          </Grid>
