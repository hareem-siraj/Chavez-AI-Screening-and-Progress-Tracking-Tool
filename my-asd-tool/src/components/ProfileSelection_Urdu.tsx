...
const handleSelectChild = async (ChildID: number) => {
  ...
  try {
    ...
    if (sessions.length === 0) {
      ...
      return navigate("/dashboard-urdu");
    }
    ...
    if (mostRecentComplete) {
      ...
      if (diffDays >= 90) {
        const proceed = window.confirm("آخری سیشن کو 90 دن سے زیادہ ہو چکے ہیں۔ کیا آپ نیا سیشن شروع کرنا چاہتے ہیں؟ اگر نہیں تو آپ کا پچھلا سیشن لوڈ ہو جائے گا۔ آپ کا انتخاب مستقل نہیں ہے آپ اپنے اگلے سائن ان پر ایک نیا سیشن شروع کر سکتے ہیں۔");
